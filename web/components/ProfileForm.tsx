...
const field =
  "w-full rounded-tile border border-seam bg-sunk min-h-[44px] px-4 py-3.5 text-[1.125rem] text-ink placeholder:text-dust outline-none transition focus:border-accent focus:shadow-[0_0_0_3px_rgb(var(--sky)/0.18)]";
...
const legend = "mb-2.5 block text-[1.0625rem] font-semibold text-ink";
...
export default function ProfileForm({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-8">
      ...
      <div className="rounded-panel border border-edge bg-sunk p-5">
        <div className="flex items-center gap-3.5">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 break-all font-mono text-[1.0625rem] text-ink">{user.email}</p>
          </div>
        </div>
          ...
          <>
            ...
            <ul className="mt-4 flex flex-wrap gap-2">
              {[batch.label, batch.branch, batch.yearLabel, `Roll ${batch.roll}`].map((v) => (
                <li
                  key={v}
                  className="rounded-full border border-seam bg-raise px-3 py-1 font-mono text-[1rem] text-haze"
                >
                  {v}
                </li>
              ))}
            </ul>
            <p className="mt-3 text-[1rem] leading-relaxed text-dust">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <div className="relative">
            <span
              aria-hidden
              className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 font-mono text-[1.125rem] text-dust"
            >
              @
            </span>
            <input
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {HOSTELS.map((h, i) => (
            ...
            >
              ...
              />
              <span className="text-[1.125rem] text-ink">{h.label}</span>
            </label>
          ))}
        </div>
        <p className="mt-2.5 text-[1rem] leading-relaxed text-dust">
      ...
      {effectivePath && (
        <p className="rounded-tile border border-dashed border-seam px-4 py-3 text-[1.0625rem] leading-relaxed text-dust">
          You arrived from{" "}
