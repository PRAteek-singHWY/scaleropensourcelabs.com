      ...
      <span className="flex w-full flex-col rounded-panel border border-seam bg-sunk p-5 transition hover:border-accent/50 peer-checked:border-accent peer-checked:bg-raise peer-checked:shadow-[0_0_0_3px_rgb(var(--sky)/0.18)] peer-focus-visible:border-accent peer-focus-visible:shadow-[0_0_0_3px_rgb(var(--sky)/0.35)]">
        <span className="flex items-start justify-between gap-3">
          <span className="min-w-0">
            ...
            {mentor.org && (
              <span className="mt-0.5 block font-mono text-[1rem] text-dust">
                {mentor.org}
              </span>
            )}
          </span>
        ...
        <span className="mt-3 block text-[1.0625rem] leading-relaxed text-haze">
          {mentor.description}
        </span>
...
export default function MentorPicker({ user }: { user: User }) {
  ...
  return (
    <div className="card rounded-panel bg-raise p-6 sm:p-8">
      ...
      {error && (
        <p className="mt-4 text-[1.0625rem] leading-relaxed text-ember" role="alert">
          {error}
        </p>
      )}
            ...
            {[
              ...
              >
                <span
                  aria-hidden
                  className={[
                    "grid h-8 w-8 shrink-0 place-items-center rounded-full font-mono text-[0.9583rem] font-bold leading-none",
                    filled
                      ? "bg-accent text-bg"
                ...
                <span
                  className={`text-[1.125rem] ${filled ? "font-semibold text-ink" : "text-dust"}`}
                >
                  {label as string}
                </span>
              </li>
      ...
      {!enrollment && !open && (
        <>
          ...
          {choosable.length === 0 ? (
            ...
            <p className="mt-6 rounded-tile border border-dashed border-seam p-5 text-[1.0625rem] leading-relaxed text-dust">
      ...
      {open && (
        <form onSubmit={onSubmit} className="mt-7">
                ...
                <li key={n} className="flex items-center gap-3">
                  <span className="flex items-center gap-2">
                    <span
                      aria-hidden
                      className={[
                        "grid h-7 w-7 shrink-0 place-items-center rounded-full font-mono text-[0.9583rem] font-bold leading-none",
                        done || live
                          ? "bg-accent text-bg"
                          : "border border-dashed border-seam text-dust",
                      ].join(" ")}
          ...
          {state === "error" && (
            <p className="mt-5 text-[1.0625rem] leading-relaxed text-ember" role="alert">
              {message}{" "}
